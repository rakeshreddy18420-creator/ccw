...
const ColHome = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await api.get('/collaborator/search', {
        ...
        const jobsWithDetails = await Promise.all(
          response.data.map(async (job) => {
            try {
              return {
                id: job.id,
                title: job.name || job.skill_category,
                full_description: job.about || `${job.skill_category} with ${job.experience} experience located in ${job.location}.`,
                meta: `Fixed-price - ${job.experience || 'Intermediate'} - Est. Budget: $${job.pricing_amount || '2,000'} - Posted Recently`,
                rateType: "$ Fixed Rate",
                rating: "★★★★☆",
                reviews: "4/5 (12 Reviews)",
                flag: getFlagForCountry(job.location),
                location: job.location || "Remote",
                creator_name: job.name || "Anonymous",
                posted_at: new Date().toISOString(),
                budget_type: 'fixed',
                expertise_level: job.experience || 'medium',
                budget_from: job.pricing_amount || 2000,
                budget_to: job.pricing_amount || 2000,
              };
            } catch (error) {
