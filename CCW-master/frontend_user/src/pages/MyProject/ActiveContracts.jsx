...
export default function ActiveContracts() {
  ...
  const [inProgressContracts, setInProgressContracts] = useState([]); // Changed from activeContracts
  ...
  const fetchContracts = async () => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        // Filter in_progress contracts (status = "in_progress") - CHANGED
        const inProgress = contractsData.filter(contract => contract.status === "in_progress");
        setInProgressContracts(inProgress);
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex gap-10 text-[16px] md:text-[20px] font-semibold mb-6 border-b border-gray-300">
              ...
              >
                Active contracts ({statusCounts.in_progress || inProgressContracts.length}) {/* CHANGED */}
              ...
              <button
                ...
                className="relative pb-4"
              >
                Awaiting contracts ({statusCounts.active || contracts.filter(c => c.status === "active").length}) {/* CHANGED */}
              </button>
            </div>

            <div className="w-full h-[1px] bg-gray-300 mb-6" />

            {/* IN_PROGRESS CONTRACTS GRID - CHANGED */}
              ...
              <div className="relative grid grid-cols-1 md:grid-cols-3 gap-8">
                {inProgressContracts.map((contract, i) => (
                  <div key={contract.id} className="relative pr-8">
                    ...
                    {i < 2 && inProgressContracts.length > 1 && (
                      <div
